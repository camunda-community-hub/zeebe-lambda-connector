  ...
  stages {
    ...
    stage('Build') {
      ...
      steps {
        ...
        container('docker') {
            sh 'docker build -t camunda/zeebe-lambda-worker:SNAPSHOT .'
        }
      }
    ...
    stage('Upload') {
      ...
      steps {
        ...
        container('docker') {
            sh 'docker push camunda/zeebe-lambda-worker:SNAPSHOT'
        }
      }
    }
    ...
    stage('Release') {
      ...
      steps {
        ...
        container('docker') {
            sh 'docker build -t camunda/zeebe-lambda-worker:SNAPSHOT .'
            sh 'docker tag camunda/zeebe-lambda-worker:SNAPSHOT camunda/zeebe-lambda-worker:${RELEASE_VERSION}'
            sh 'docker push camunda/zeebe-lambda-worker:${RELEASE_VERSION}'
            sh 'docker tag camunda/zeebe-lambda-worker:SNAPSHOT camunda/zeebe-lambda-worker:latest'
            sh 'docker push camunda/zeebe-lambda-worker:latest'
        }
      }
    }
  }
